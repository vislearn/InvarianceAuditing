...
# ConvNeXt V2-tiny CheXpert (seed 1)
...
invariances (Figure 18) — so if you want one classifier, take this one; if you
want to reproduce that experiment, you need both.
...
model = AutoModelForImageClassification.from_pretrained(
    "RussellALA/convnext-chexpert-seed1").eval()
...
model = ConvNextClassfierSubjectModel("RussellALA/convnext-chexpert-seed1",
                                      n_channels=1).eval()
```
...
loss** and nearest-neighbour baseline in Table 5, and its cross-model agreement
with BiomedCLIP in Figure 8 — measurements of what the model treats as
equivalent, not of its diagnostic accuracy.
